module TimeTracker
  class Event
    attr_accessor :task_id, :start_time, :end_time, :date, :duration, :title, :description

    def self.build_from(external_event)
      if external_event.is_a? Google::Apis::CalendarV3::Event
        event = self.new
        event.task_id = MEETING_TASK_ID
    ...
    def == other
      unless other.instance_of? Event
        return false
      end
      task_id == other.task_id && spent_date == other.spent_date && title == other.title && hours == other.hours
    end
  end
end
